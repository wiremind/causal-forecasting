...
class MIMIC3RealDataset(Dataset):
    ...
    def __init__(
        ...
        subject_ids = (
            treatments.index.get_level_values("subject_id")
    ...
    def process_sequential(
        self, encoder_r, projection_horizon, encoder_outputs=None, save_encoder_r=False
    ):
        """
    ...
    def process_autoregressive_test(
        ...
        if not self.processed_autoregressive:
            ...
            logger.info(
                f"{self.subset_name} Dataset already processed (autoregressive)"
            )
